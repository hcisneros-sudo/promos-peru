...
def query_geoapify(query: str, api_key: str) -> dict | None:
    params = urllib.parse.urlencode({"text": query, "filter": "countrycode:pe", "format": "json", "limit": 1, "apiKey": api_key})
    request = urllib.request.Request(f"https://api.geoapify.com/v1/geocode/search?{params}", headers={"User-Agent": "PromosPeru/1.0"})
...
def geocode_locations(promotions: list[dict], cache: dict, api_key: str, requery_all: bool) -> tuple[int, int]:
    requested = reused = 0
    for promotion in promotions:
        for location in promotion.get("ubicacion", {}).get("locales", []):
            ...
            result = None if requery_all else cache.get(key)
            if result:
                reused += 1
            elif api_key:
...
def compact(master: dict, history: dict[str, dict]) -> tuple[list[dict], int]:
    catalog = []
    migrated = 0
    for p in master.get("promociones", []):
        ...
        for source in p.get("ubicacion", {}).get("locales", []):
            ...
            previous = history.get(normalized(address))
...
def write_catalog(catalog: list[dict], output: Path, part_size: int, migrated: int, geocode_stats: tuple[int, int]) -> dict:
    ...
    manifest = {"version": 3, "generatedAt": datetime.now(timezone.utc).isoformat(), "source": "ChatGPT Work master",
                "promotions": len(catalog), "mappedLocations": mapped, "migratedCoordinates": migrated,
                "geoapifyRequested": geocode_stats[0], "geoapifyReused": geocode_stats[1],
                "uncompressedBytes": len(raw), "compressedBytes": len(compressed), "sha256": hashlib.sha256(compressed).hexdigest(), "parts": names}
...
def main() -> int:
    ...
    history_applied = apply_history_to_master(master.get("promociones", []), history)
    cache = json.loads(args.cache.read_text(encoding="utf-8")) if args.cache.exists() else {}
    api_key = os.environ.get("GEOAPIFY_API_KEY", "")
    geocode_stats = geocode_locations(master.get("promociones", []), cache, api_key, args.regeocodificar_todo)
    args.cache.parent.mkdir(parents=True, exist_ok=True)
    args.cache.write_text(json.dumps(cache, ensure_ascii=False, indent=2), encoding="utf-8")
    catalog, migrated = compact(master, history)
