...
const state = {promotions:[],filtered:[],visible:60,user:null,map:null,layer:null,detailMap:null,deferredInstall:null,view:"list",favorites:new Set(),scrollY:0,selected:Object.fromEntries(FILTER_KEYS.map(k=>[k,new Set()])),discountActive:false,mapFocusUser:false};
...
const fold=v=>clean(v).normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLocaleLowerCase("es");
...
const visiblePromotions=()=>state.filtered.slice(0,state.visible);
...
function renderMulti(key){
  const values=filterValues(key),selected=state.selected[key];
  for(const value of [...selected])if(!values.includes(value))selected.delete(value);
  const target=$(`${key}Options`);
  const options=values.length?values.map(value=>`<label data-filter-option><input type="checkbox" value="${escapeHtml(value)}" ${selected.has(value)?"checked":""}><span>${escapeHtml(value)}</span></label>`).join(""):`<p>Sin opciones disponibles</p>`;
  target.innerHTML=`<div class="filter-search-wrap"><input class="filter-search-input" type="search" placeholder="Buscar..." aria-label="Buscar dentro del filtro"></div>${options}`;
  const search=target.querySelector(".filter-search-input");
  if(search)search.addEventListener("input",()=>{const q=fold(search.value);target.querySelectorAll("[data-filter-option]").forEach(label=>{label.hidden=!!q&&!fold(label.textContent).includes(q);});});
  target.querySelectorAll('input[type="checkbox"]').forEach(input=>input.addEventListener("change",()=>{input.checked?selected.add(input.value):selected.delete(input.value);if(["category","department","province"].includes(key))populateFilters();updateFilterLabels();applyFilters();}));
}
...
function applyFilters(preserveVisible=false){
  ...
  if(!preserveVisible)state.visible=60;
  render();
}
...
function render(){
  ...
  if(mapMode){
    ...
    const locations=state.filtered.flatMap(matchingLocations).filter(physical);
    const mapped=locations.filter(l=>Number.isFinite(l.lat)&&Number.isFinite(l.lng));
    $("mapCount").textContent=`${mapped.length.toLocaleString("es-PE")} locales en mapa`;
    renderMap();
    return;
  }

  const shown=visiblePromotions();
  $("mapView").hidden=true;
  $("listView").hidden=false;
  $("listView").innerHTML=shown.map(card).join("")||`<div class="state">${state.view==="favorites"?"Aún no guardaste promociones favoritas.":"No encontramos promociones con esos filtros."}</div>`;
  $("moreButton").hidden=state.visible>=state.filtered.length;
  const locations=state.filtered.flatMap(matchingLocations).filter(physical);
  const mapped=locations.filter(l=>Number.isFinite(l.lat)&&Number.isFinite(l.lng));
  $("mapCount").textContent=`${mapped.length.toLocaleString("es-PE")} locales en mapa`;
  document.querySelectorAll(".card").forEach(el=>el.addEventListener("click",()=>openDetail(state.promotions.find(p=>String(p.id)===el.dataset.id),false)));
...
function renderMap(){
  ...
  addUserMarker(state.layer);$("mapLimit").textContent=points.length?`Mostrando ${points.length.toLocaleString("es-PE")} locales del resultado actual.`:"";
  setTimeout(()=>{state.map.invalidateSize();if(state.mapFocusUser&&state.user)state.map.setView(state.user,14);else if(points.length)state.map.fitBounds(L.latLngBounds(points.map(({l})=>[l.lat,l.lng])),{padding:[24,24],maxZoom:14});},100);
}
...
function switchView(view){state.view=view;$("listTab").classList.toggle("active",view==="list");$("mapTab").classList.toggle("active",view==="map");$("favoritesTab").classList.toggle("active",view==="favorites");applyFilters(true);}
function clearFilters(){FILTER_KEYS.forEach(key=>state.selected[key].clear());state.discountActive=false;$("discountMin").value=0;$("discountMax").value=100;$("query").value="";state.user=null;state.mapFocusUser=false;$("message").textContent="";updateDiscount();populateFilters();applyFilters();}
...
function closeOpenFiltersOutside(target){if(!target.closest(".multi-filter"))document.querySelectorAll(".multi-filter[open]").forEach(d=>d.removeAttribute("open"));if(!target.closest(".advanced"))document.querySelectorAll(".advanced[open]").forEach(d=>{d.querySelectorAll(".multi-filter[open]").forEach(inner=>inner.removeAttribute("open"));d.removeAttribute("open");});}
function bind(){
  ...
  $("nearButton").onclick=()=>getUserLocation(()=>{$("message").textContent="Promociones ordenadas desde la ubicación más cercana.";applyFilters();if(state.view==="map"){state.mapFocusUser=true;renderMap();}});$("mapLocateButton").onclick=()=>getUserLocation(()=>{state.mapFocusUser=true;$("message").textContent="Mapa centrado en tu ubicación.";renderMap();});
  for(const id of ["discountMin","discountMax"])$(id).addEventListener("input",()=>{state.discountActive=true;updateDiscount();applyFilters();});
  document.addEventListener("click",e=>{const button=e.target.closest("[data-map-detail]");if(button)openDetail(state.promotions.find(p=>String(p.id)===button.dataset.mapDetail),true);closeOpenFiltersOutside(e.target);});
}
